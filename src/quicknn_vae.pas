...
type
  ...
  TVAE = record
      ...
      function decode(const latent: PQNNFloat; const batch, latent_h, latent_w: longint):TQNNImage;   overload;
      procedure decode(const dst, latent: PQNNFloat; const batch, latent_h, latent_w: longint);       overload;
      procedure load(var f: file);                                    overload;
...
function TVAE.decode(const latent: PQNNFloat; const batch, latent_h,
...
procedure TVAE.decode(const dst, latent: PQNNFloat; const batch, latent_h, latent_w: longint);
const ch_mult : array[0..3] of integer = (1, 2, 4, 4);
var
    x, work, mean_ptr, var_ptr: PQNNFloat;
...
begin
    ...
    ch_mult := QNN_VAE_CH_MULT;
    enc_conv_in_weight := sf.getTensorDataMemBlock('encoder.conv_in.weight', false);
    enc_conv_in_bias := sf.getTensorDataMemBlock('encoder.conv_in.bias', false);
            ...
            for r := 0 to num_res_blocks -1 do
                begin
                    if ((r = 0) and (level > 0)) then
                        ...
                        in_ch := ch;
                    enc_down_blocks[block_idx].load(sf, format('encoder.down_blocks.%d.resnets.%d', [level, r]), in_ch, ch);
                    inc(block_idx)
                end
        end;
    setLength(enc_downsample, 3);
    for i := 0 to high(enc_downsample) do
        begin
            ch := base_channels * ch_mult[i];
            enc_downsample[i].channels := ch;
            enc_downsample[i].conv_weight := sf.getTensorDataMemBlock(format('encoder.down_blocks.%d.downsamplers.0.conv.weight', [i]), false);
            enc_downsample[i].conv_bias   := sf.getTensorDataMemBlock(format('encoder.down_blocks.%d.downsamplers.0.conv.bias', [i]), false)
        end;
    mid_ch := base_channels * ch_mult[3];
    enc_mid_block1.load(sf, 'encoder.mid_block.resnets.0', mid_ch, mid_ch);
    enc_mid_attn.load(sf, 'encoder.mid_block.attentions.0', mid_ch);
    enc_mid_block2.load(sf, 'encoder.mid_block.resnets.1', mid_ch, mid_ch);
    enc_norm_out_weight := sf.getTensorDataMemBlock('encoder.conv_norm_out.weight', false);
    enc_norm_out_bias := sf.getTensorDataMemBlock('encoder.conv_norm_out.bias', false);
    enc_conv_out_weight := sf.getTensorDataMemBlock('encoder.conv_out.weight', false);
    enc_conv_out_bias := sf.getTensorDataMemBlock('encoder.conv_out.bias', false);
    if assigned(sf.getTensor('quant_conv.weight')) then
        begin
            quant_conv_weight := sf.getTensorDataMemBlock('quant_conv.weight', false);
            quant_conv_bias := sf.getTensorDataMemBlock('quant_conv.bias', false)
        end;
    dec_conv_in_weight := sf.getTensorDataMemBlock('decoder.conv_in.weight', false);
    dec_conv_in_bias := sf.getTensorDataMemBlock('decoder.conv_in.bias', false);
    dec_mid_block1.load(sf, 'decoder.mid_block.resnets.0', mid_ch, mid_ch);
    dec_mid_attn.load(sf, 'decoder.mid_block.attentions.0', mid_ch);
    dec_mid_block2.load(sf, 'decoder.mid_block.resnets.1', mid_ch, mid_ch);
        ...
        for r := 0 to num_res_blocks+1 -1 do
            begin
                if (r = 0) then
                    ...
                    in_ch := ch;
                up_idx := 3-level;
                dec_up_blocks[block_idx].load(sf, format('decoder.up_blocks.%d.resnets.%d', [up_idx, r]), in_ch, ch);
                inc(block_idx);
            end;
        dec(level)
    end;
    setLength(dec_upsample, 3);
    for i := 0 to high(dec_upsample) do
      begin
        ch := base_channels * ch_mult[3-i];
        dec_upsample[i].channels := ch;
        dec_upsample[i].conv_weight := sf.getTensorDataMemBlock(format('decoder.up_blocks.%d.upsamplers.0.conv.weight', [i]), false);
        dec_upsample[i].conv_bias := sf.getTensorDataMemBlock(format('decoder.up_blocks.%d.upsamplers.0.conv.bias', [i]), false)
      end;
    dec_norm_out_weight := sf.getTensorDataMemBlock('decoder.conv_norm_out.weight', false);
    dec_norm_out_bias   := sf.getTensorDataMemBlock('decoder.conv_norm_out.bias', false);
    dec_conv_out_weight := sf.getTensorDataMemBlock('decoder.conv_out.weight', false);
    dec_conv_out_bias   := sf.getTensorDataMemBlock('decoder.conv_out.bias', false);
    bn_mean_t := sf.getTensor('bn.running_mean');
    if assigned(bn_mean_t) then
        begin
            bn_mean := bn_mean_t.asSingle;
            bn_var_t := sf.getTensor('bn.running_var');
            if assigned(bn_var_t) then
                bn_var := bn_var_t.asSingle
            else
                bn_var := nil
        end;
    if assigned(sf.getTensor('post_quant_conv.weight')) then
        begin
            post_quant_conv_weight := sf.getTensorDataMemBlock('post_quant_conv.weight', false);
            post_quant_conv_bias := sf.getTensorDataMemBlock('post_quant_conv.bias', false)
        end;
    if not boolean(bn_mean) and (scaling_factor = 0.0) then
        begin
            lc := latent_channels;
