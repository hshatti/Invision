...
type
   ...
   private
     ...
     function GetSlowDeceaseSpeed: integer;
     procedure SetColor(const Value: TColor);
     procedure SetBorder(const Value: Byte);
     procedure SetDrawFocus(AValue: boolean);
     procedure SetSlowDeceaseSpeed(AValue: integer);
...
//      procedure WMPaint(var Message: TWMPaint); message WM_PAINT;
   ...
   published
      ...
      property ColorStyle: THSColorStyle read FDummyStyle write SetColorStyle default lcsAqua;
...
//      property Enabled: boolean read FEnabled write SetEnabled default true;
      ...
      property Layout: THSButtonLayout read FLayout write SetLayout default THSButtonLayout.blGlyphLeft;
      ...
      property SlowDeceaseSpeed : integer read GetSlowDeceaseSpeed write SetSlowDeceaseSpeed;
...
//   FColorWhenDown := clNone;
   ...
   FDefault := false;
   FCancel := false;
   FDummyStyle := lcsDefault;
   GetPreDefinedColors(lcsDefault, FColor, FLightColor, FShadowColor, FColorWhenDown, FHotTrackColor, FontColor, FFlat, bModern, bQuicken);
...
begin
  if FDrawFocus=AValue then Exit;
  FDrawFocus:=AValue;
end;

procedure THSButton.SetSlowDeceaseSpeed(AValue: integer);
begin
  if assigned(FTimer) and (FTimer.Interval<>AValue) then FTimer.Interval:=AValue;
end;
...
function THSButton.GetSlowDeceaseSpeed: integer;
begin
  result :=0;
    if assigned(FTimer) then
...
//   Color := FColor;
   Color:=FColor;
//   HotTrackColor:=FHotTrackColor;
//   LightColor:=FLightColor;
//   ShadowColor:=FShadowColor;
//   ColorWhenDown:=FColorWhenDown;
   Font.Color := FontColor;
...
begin
   ...
   begin
      ...
      {$ifdef FPC}
        aBitmap.Canvas.Brush.Color := Parent.Color;
        aBitmap.TransparentColor := Parent.Color;
      {$else}
   ...
   {$endif}
   {$ifdef WINDOWS}
     BitBlt(Canvas.Handle,Canvas.Cliprect.Left,Canvas.Cliprect.Top,Canvas.Cliprect.Right,Canvas.Cliprect.Bottom,aBitmap.Canvas.Handle,Canvas.Cliprect.Left,Canvas.Cliprect.Top,SRCCOPY);
   //Canvas.Draw(0,0, aBitmap);// this is showing some flickering behavior on windws replacing with direct API BitBlt
   {$else}
   Canvas.Draw(0,0, aBitmap);// this is showing some flickering behavior on windws replacing with direct API BitBlt
   {$endif}
   aFont.Free;
   ...
   if focused and enabled and (FDrawFocus) and (FStyle <> bsShape) then Canvas.DrawFocusRect(Rect(4, 4, Width-4, Height - 4));
